...
function transformProductToDesign(product: any): Design {
  ...
  // Get thumbnail - prioritize AVIF files (mockup AVIF first, then AVIF JPG/PNG, then mockup, then JPG/PNG)
  ...
  if (product.media_files && product.media_files.length > 0) {
    // First, try to find AVIF mockup
    const avifMockupMedia = product.media_files.find((m: any) => 
      (m.is_avif === true || (m.file || m.url || '').toLowerCase().endsWith('.avif')) && 
      m.is_mockup === true
    );
    if (avifMockupMedia) {
      thumbnailUrl = avifMockupMedia.file || avifMockupMedia.url || thumbnailUrl;
    } else {
      // Second, try to find AVIF JPG/PNG
      const avifJpgPngMedia = product.media_files.find((m: any) => 
        (m.is_avif === true || (m.file || m.url || '').toLowerCase().endsWith('.avif')) && 
        m.is_jpg_png === true
      );
      if (avifJpgPngMedia) {
        thumbnailUrl = avifJpgPngMedia.file || avifJpgPngMedia.url || thumbnailUrl;
      } else {
        // Third, try to find a mockup image (non-AVIF)
        ...
        if (mockupMedia) {
          ...
          // Fourth, try to find a JPG/PNG image (non-AVIF)
...
export const InstagramAPI = {
  ...
  /**
   * Check if product image is under Meta 8 MB limit for Instagram.
   */
  async checkImage(productId: string, mediaType: 'mockup' | 'jpg' | 'png'): Promise<ApiResponse<{
    ok: boolean;
    size_bytes: number | null;
    max_bytes: number;
    error?: string | null;
  }>> {
    return apiClient.get(`api/common/instagram/check-image/?product_id=${encodeURIComponent(productId)}&media_type=${encodeURIComponent(mediaType)}`);
  },
...
export const SystemConfigAPI = {
    ...
    if (response.data) {
      return {
        ...
        data: {
          ...
          freeDesignsPerAccountOneTime: response.data.free_designs_per_account_one_time ?? 10,
          freeCustomOrdersPerAccount: response.data.free_custom_orders_per_account ?? 2,
  ...
  async updateSystemConfig(data: Partial<SystemConfig>): Promise<ApiResponse<SystemConfig>> {
    ...
    const backendData: any = {
      ...
      free_designs_per_account_one_time: data.freeDesignsPerAccountOneTime,
      free_custom_orders_per_account: data.freeCustomOrdersPerAccount,
